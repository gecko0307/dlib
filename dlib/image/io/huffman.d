...
void treeAddCode(HuffmanTreeNode* root, HuffmanCode code, ubyte value)
{
    ...
    foreach(bit; code.toString)
    {
        if (bit == '0')
        {
            if (node.left is null)
            {
                node.left = new HuffmanTreeNode(null, null, 0, 0, false);
        ...
        else if (bit == '1')
        {
            ...
            {
                node.right = new HuffmanTreeNode(null, null, 0, 0, false);
...
HuffmanTreeNode* treeFromTable(ubyte[HuffmanCode] table)
{
    HuffmanTreeNode* root = new HuffmanTreeNode(null, null, 0, 0, false);

    foreach(i, v; table)
        treeAddCode(root, i, v);
...
unittest
{
    ...
    ubyte[] enc = encodeHuffman(cast(ubyte[])str, tree);

    BitWriter bw;
    packHuffmanTree(tree, &bw);
    ubyte[] treeData = bw.getData();

    BitReader br = BitReader(treeData);
    HuffmanTreeNode* utree = unpackHuffmanTree(&br);

    ubyte[] dec = decodeHuffman(enc, utree);
    auto decStr = cast(char[])dec;
