...
class SelectorStreamTransport : StreamTransport
{
    ...
    this(Loop loop, Socket socket, Protocol protocol = null)
    {
        ...
        output_ = MmapPool.instance.make!ReadBuffer();
    }
    ...
    bool receive()
    {
        auto readCount = recv(socket.handle, output[].ptr, output.free, 0);

        if (readCount > 0)
        {
            output_ ~= output[0..readCount];
            return false;
        }
        else if (readCount == 0)
        {
    ...
    body
    {
        auto sentCount = .send(socket.handle,
                               input[].ptr,
                               input.length,
                               0);
