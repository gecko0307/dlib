...
# Modified by Timur Gafarov for using in dlib project (http://github.com/gecko0307/dlib)
...
class Markdown(object):
    ...
    def _do_link_patterns(self, text):
        ...
        """
        link_from_hash = {}
        for regex, repl, linktext in self.link_patterns:
            replacements = []
            ...
            for match in regex.finditer(text):
                if hasattr(repl, "__call__"):
                    ...
                    if len(linktext) > 0:
                        linktext = match.expand(linktext)
            ...
            for (start, end), href in reversed(replacements):
                ...
                link = '<a href="%s">%s</a>' % (escaped_href, linktext)
    ...
    if opts.link_patterns_file:
        ...
        try:
            for i, line in enumerate(f.readlines()):
                ...
                try:
                    pat, href, text = line.rstrip().rsplit(None, 2)
                except ValueError:
                    raise MarkdownError("%s:%d: invalid link pattern line: %r"
                                        % (opts.link_patterns_file, i+1, line))
                link_patterns.append(
                    (_regex_from_encoded_pattern(pat), href, text))
        finally:
            f.close()
    else:
        link_patterns = None
